...
public class PForDocVector {
	...
	public static void fromIntArrayWritable(IntArrayWritable in,
			PForDocVector doc) {
		try {
			...
			if (doc.termids.length < 128) {
				VB.uncompress(compressed, inPos, in.size() - 1, doc.termids,
						outPos);
				return;
			}
			...
			VB.uncompress(compressed, inPos, in.size() - inPos.get(),
					doc.termids, outPos);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void toIntArrayWritable(IntArrayWritable ints, int[] termids,
			int length) {
