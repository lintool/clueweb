...
public class BuildDictionary extends Configured implements Tool {
	...
	private static class MyReducer extends
		...
		private FSDataOutputStream termsOut, idsOut, idsToTermOut, dfByTermOut,
				cfByTermOut, dfByIntOut, cfByIntOut;
		...
		public void setup(
			...
			idsToTermOut = fs.create(new Path(basePath, TERMS_ID_MAPPING_DATA),
					true);
		...
		public void reduce(Text key, Iterable<PairOfIntLong> values,
			...
			if (iter.hasNext()) {
				throw new RuntimeException("More than one record for term: "
						+ term);
			}
		...
		public void cleanup(
			...
			QuickSort
					.quicksortWithSecondary(seqNums, dfs, cfs, 0, numTerms - 1);
			...
			ShiftAddXorSignedStringMap dict = new ShiftAddXorSignedStringMap(
					...
					new TwoStepsLcpMonotoneMinimalPerfectHashFunction<CharSequence>(
							termList, DictionaryTransformationStrategy
									.getStrategy()));
	...
	public int run(String[] args) throws Exception {
		...
		Job job = new Job(conf, BuildDictionary.class.getSimpleName() + ":"
				+ input);
	...
	public static void main(String[] args) throws Exception {
		LOG.info("Running " + BuildDictionary.class.getCanonicalName()
