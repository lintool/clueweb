...
public class DumpWarcRecordsToPlainText extends Configured implements Tool {
	private static final Logger LOG = Logger
			.getLogger(DumpWarcRecordsToPlainText.class);
			...
			if (docid != null) {
				...
				try {
					...
					String cleaned = Jsoup.parse(doc.getContent()).text()
							.replaceAll("[\\r\\n]+", " ");
					cleaned = JOINER.join(AnalyzerUtils
	...
	public int run(String[] args) throws Exception {
		...
		LOG.info("Tool name: "
				+ DumpWarcRecordsToPlainText.class.getSimpleName());
		LOG.info(" - input: " + input);
		LOG.info(" - output: " + output);

		JobConf conf = new JobConf(getConf(), DumpWarcRecordsToPlainText.class);
		conf.setJobName(DumpWarcRecordsToPlainText.class.getSimpleName() + ":"
				+ input);
	...
	public static void main(String[] args) throws Exception {
		LOG.info("Running "
				+ DumpWarcRecordsToPlainText.class.getCanonicalName()
