...
public class JelineckMercerLMRetrieval extends Configured implements Tool {
  private static final Logger LOG = Logger.getLogger(JelineckMercerLMRetrieval.class);

  /*
   * Mapper outKey: query id (integer) Mapper outValue: two part element: documentid probability
   */
    ...
    public void setup(Context context) throws IOException {
      ...
      while ((line = br.readLine()) != null) {
        ...
        for (String term : terms) {
          ...
          cfMap.put(termid, -1l);
          if (termid < 0) {
            LOG.info("Query " + qid + ": term [" + term + "] not found in the provided dictionary!");
            continue;
          }
  ...
  private static class MyReducer extends Reducer<IntWritable, PairOfStringFloat, IntWritable, Text> {
    ...
    public void reduce(IntWritable key, Iterable<PairOfStringFloat> values, Context context)
      ...
      // in-memory hack: keep a hashmap of all docids and aggregate the log probs
  ...
  public int run(String[] args) throws Exception {
    ...
    if (!cmdline.hasOption(VBDOCVECTOR_OPTION) || !cmdline.hasOption(OUTPUT_OPTION)
        || !cmdline.hasOption(DICTIONARY_OPTION) || !cmdline.hasOption(QUERIES_OPTION)
        || !cmdline.hasOption(COLLECTION_STATS) || !cmdline.hasOption(LAMBDA)
    ...
    LOG.info("Tool name: " + JelineckMercerLMRetrieval.class.getSimpleName());
    ...
    Job job = new Job(conf, JelineckMercerLMRetrieval.class.getSimpleName() + ":" + vbdocvector);
    job.setJarByClass(JelineckMercerLMRetrieval.class);
  ...
  public static void main(String[] args) throws Exception {
    LOG.info("Running " + JelineckMercerLMRetrieval.class.getCanonicalName() + " with args "
        + Arrays.toString(args));
    ToolRunner.run(new JelineckMercerLMRetrieval(), args);
  }
}
