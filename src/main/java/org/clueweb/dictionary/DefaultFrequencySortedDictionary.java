...
/**
 * An implementation of {@link FrequencySortedDictionary}. Term ids start at 1,
 * which corresponds to the most frequent term. Term id 2 is the second most
 * frequent term, etc.
 * 
 * @author Jimmy Lin
 */
public class DefaultFrequencySortedDictionary implements
	 ...
	 */
	public DefaultFrequencySortedDictionary(String basePath, FileSystem fs)
			throws IOException {
	...
	public static void main(String[] args) throws Exception {
		...
		DefaultFrequencySortedDictionary dictionary = new DefaultFrequencySortedDictionary(
				path, fs);
		...
		out.println("max df = " + stats.getMaxDf() + ", termid "
				+ stats.getMaxDfTerm());
		out.println("max cf = " + stats.getMaxCf() + ", termid "
				+ stats.getMaxCfTerm());
		...
		while ((cmd = stdin.readLine()) != null) {
			...
			if (tokens[0].equals("termid")) {
				...
				out.println("termid=" + termid + ", term="
						+ dictionary.getTerm(termid));
				out.println("  df = " + stats.getDf(termid) + ", cf = "
						+ stats.getCf(termid));
			} else if (tokens[0].equals("term")) {
				String term = tokens[1];

				out.println("term=" + term + ", termid="
