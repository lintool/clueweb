...
public class QuickSort {
	...
	// quicksort a[left] to a[right]
	public static void quicksort(int[] keys, int[] counts, short[] counts2,
			int left, int right) {
		if (right <= left)
			return;
	...
	public static void quicksort(short[] keys, int[] counts, Object[] counts2,
			int left, int right) {
		if (right <= left)
			return;
		int i = partition(keys, counts, counts2, left, right);
	...
	public static void quicksortWithSecondary(int[] keys, int[] counts,
			short[] counts2, int left, int right) {
		if (right <= left)
			return;
	...
	public static void quicksortWithSecondary(int[] keys, int[] counts,
			long[] counts2, int left, int right) {
		if (right <= left)
			return;
		int i = partitionWithSecondary(keys, counts, counts2, left, right);
		quicksortWithSecondary(keys, counts, counts2, left, i - 1);
	...
	public static void quicksort(Object[] keys, int[] counts, int left,
			int right) {
		if (right <= left)
			return;
		int i = partition(keys, counts, left, right);
	...
	private static int partition(short[] keys, int[] counts, int left, int right) {
		...
		int j = right;
		while (true) {
			while (counts[++i] < counts[right])
				// find item on left to swap
				; // a[right] acts as sentinel
			while (counts[right] < counts[--j])
	...
	private static int partition(Object[] keys, int[] counts, int left,
			int right) {
		int i = left - 1;
		int j = right;
		while (true) {
	...
	private static int partition(int[] keys, int[] counts, short[] counts2,
			int left, int right) {
		int i = left - 1;
		int j = right;
		while (true) {
	...
	private static int partition(short[] keys, int[] counts, Object[] counts2,
			int left, int right) {
		int i = left - 1;
		int j = right;
		while (true) {
	...
	private static int partitionWithSecondary(int[] keys, int[] counts,
			short[] counts2, int left, int right) {
		int i = left - 1;
		int j = right;
		while (true) {
			do {
				i++;
					...
					|| (counts[i] == counts[right] && keys[i] < keys[right])) // find
																				// item
																				// on
																				// left
																				// to
			...
			do {
				j--;
					...
					&& (counts[right] < counts[j] || (counts[right] == counts[j] && keys[right] < keys[j]))); // find
																												// item
																												// on
																												// right
																												// to
